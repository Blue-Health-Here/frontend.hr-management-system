...
export default function Sidebar({ isEmployee }: SidebarProps) {
  ...
  return (
    <div className={`w-64 flex-col z-[99] fixed left-0 top-0 bg-[#1C202F] bottom-0 transition-transform duration-300 lg:translate-x-0 lg:flex ${isSidebarOpen ? 'flex shadow-2xl' : 'hidden lg:flex'}`}>
      {isSidebarOpen && <span onClick={() => dispatch(setIsSidebarOpen(false))} className='absolute top-4 right-4 text-[#F2F7FA] cursor-pointer'><RxCross2 size={20} /></span>}
        ...
        {sidebarItems.map((item, index) => {
          ...
          return (
            <div key={index}>
              {/* Headings */}
              {index === 0 && (
                <p className="text-xs font-semibold text-[#FFFFFF66] mb-1 pl-3">Dashboard</p>
              )}
              {index === 1 && (
                <p className="text-xs font-semibold text-[#FFFFFF66] mb-1 pl-3 mt-3">Pages</p>
              )}
